...
_______________________________
SSH CLOUDFRONT : $cloudfront:80@$Login:$Pass
_______________________________
Payload WS/WSS   :
...
Host Cloudfront  :  <code>$cloudfront</code>
Wildcard         :  <code>bug.$domen</code>
Login Limit      :  ${iplim} IP
Port OpenSSH    :  22
...
◇━━━━━━━━━━━━━━━━━◇
SSH CLOUDFRONT : <code>$cloudfront:80@$Login:$Pass</code>
◇━━━━━━━━━━━━━━━━━◇
...
echo -e "$COLOR1 $NC  ${WH}Host       ${COLOR1}: ${WH}bug.$domen" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
if [[ -e /etc/cloudfront ]]; then
echo -e "$COLOR1 $NC  ${WH}Cloudfront ${COLOR1}: ${WH}$cloudfront" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
fi
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 $NC  ${WH}CLOUDFRONT ${COLOR1}: ${WH}$cloudfront:80@$Login:$Pass" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
...
<code>◇━━━━━━━━━━━━━━◇</code>
<i>AutoNotif Renew Account From Server..</i>
"
curl -s --max-time $TIMES -d "chat_id=$CHATID2&disable_web_page_preview=1&text=$TEXT2&parse_mode=html" $URL2 >/dev/null
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
