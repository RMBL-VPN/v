...
_______________________________
Payload WS/WSS   : 
GET / HTTP/1.1[crlf]Host: ${domen}[crlf]Connection: Keep-Alive[crlf]User-Agent: [ua][crlf]Upgrade: ws[crlf][crlf]
_______________________________
...
echo -e "$COLOR1 $NC  ${WH}IP         ${COLOR1}: ${WH}$IP" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 $NC  ${WH}Host       ${COLOR1}: ${WH}$domen" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 $NC  ${WH}User IP    ${COLOR1}: ${WH}${iplim} IP" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
...
echo -e "$COLOR1${NC}${WH}GET / HTTP/1.1[crlf]Host: ${domen}[crlf]Connection: Keep-Alive[crlf]User-Agent: [ua][crlf]Upgrade: ws[crlf][crlf]${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
...
echo "SHELL=/bin/sh" >/etc/cron.d/trialssh$Login
echo "PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin" >>/etc/cron.d/trialssh$Login
echo "*/$timer * * * *  root /usr/bin/trialssh $Login $timer" >>/etc/cron.d/trialssh$Login

cat > /home/vps/public_html/ssh-$Login.txt <<-END
_______________________________
Format Trial SSH OVPN Account
_______________________________
Username         : $Login
Password         : $Pass
...
TEXT="
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<code>  Trial SSH Premium Account   </code>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<code>Username        : </code> <code>$Login</code>
<code>Password        : </code> <code>$Pass</code>
<code>Expired          : </code> <code>$timer Minutes</code>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<code>IP               : </code> <code>$IP</code>
<code>ISP              : </code> <code>$ISP </code>
<code>CITY             : </code> <code>$CITY</code>
...
echo -e " $COLOR1│ $NC                                              ${NC} $COLOR1 $NC"
echo -e " $COLOR1│ $NC  ${WH}[${COLOR1}03${WH}]${NC} ${COLOR1}• ${WH}RENEW AKUN${NC}    ${WH}[${COLOR1}07${WH}]${NC} ${COLOR1}• ${WH}CHANGE USER IP LIMIT${NC}    $COLOR1 $NC"
echo -e " $COLOR1│ $NC                                              ${NC} $COLOR1 $NC"
echo -e " $COLOR1│ $NC  ${WH}[${COLOR1}04${WH}]${NC} ${COLOR1}• ${WH}DELETE AKUN${NC}   ${WH}[${COLOR1}08${WH}]${NC} ${COLOR1}• ${WH}SETTING TIME AND LOCK LOGIN${NC}    $COLOR1 $NC"
echo -e " $COLOR1│ $NC                                              ${NC} $COLOR1 $NC"
echo -e " $COLOR1│ $NC  ${WH}[${COLOR1}00${WH}]${NC} ${COLOR1}• ${WH}GO BACK${NC}       ${WH}[${COLOR1}09${WH}]${NC} ${COLOR1}• ${WH}UNLOCK USER MULTI LOGIN${NC}    $COLOR1 $NC"
echo -e " $COLOR1└───────────────────────────────────────────────┘${NC}"
